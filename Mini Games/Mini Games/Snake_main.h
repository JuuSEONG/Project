...
public:
    ...
    void move() {
        ...
        if (!ateFood) // If didn't eat food, pop back
            body.pop_back();
        else
            ateFood = false;
    }
    ...
    void grow() {
        ateFood = true; // Set flag to true indicating that snake ate food
    }
...
private:
    bool ateFood = false; // Flag to indicate if snake ate food
};
...
public:
    ...
    void loadResources() {
        ...
        restartSprite.setPosition(WIDTH / 2 - restartSprite.getGlobalBounds().width / 2, HEIGHT / 2);
    ...
    void render() {
        window.clear(Color::White); // 배경을 흰색으로 설정
        ...
        if (gameState == GAME_OVER) {
            // Draw game over sprite and restart sprite
